  ...
  async openForEdit(relationshipId) {
    ...
    try {
      ...
      this.showError(
        error.body?.message ||
          error.message ||
          "An error occurred while loading the relationship"
      );
      this.close();
    } finally {
  ...
  async handleSubmit() {
    ...
    try {
      if (this._isEditMode) {
        ...
        this.dispatchEvent(
          new CustomEvent("relationshipupdated", {
        ...
        // Dispatch event to notify parent component to refresh
        this.dispatchEvent(
          new CustomEvent("relationshipcreated", {
      ...
      this.showError(
        error.body?.message ||
          error.message ||
          "An error occurred while saving the relationship"
      );
    } finally {
      this._isLoading = false;
    }
