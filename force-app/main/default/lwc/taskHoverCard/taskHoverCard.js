...
export default class TaskHoverCard extends LightningElement {
  ...
  connectedCallback() {
    ...
    setTimeout(() => {
      ...
      let parentWrapper = this.template.host.closest(
        ".task-name-wrapper, .subtask-name-wrapper"
      );
      ...
      if (parentWrapper) {
        ...
        parentWrapper.addEventListener(
          "mouseenter",
        ...
        parentWrapper.addEventListener(
          "focusout",
          this._boundHandlers.focusout
        );
      } else {
        console.warn(
          "taskHoverCard: Could not find parent wrapper (.task-name-wrapper or .subtask-name-wrapper)"
        );
      }
    }, 0);
  }
  ...
  disconnectedCallback() {
    ...
    if (this._parentWrapper && this._boundHandlers) {
      if (this._boundHandlers.mouseenter) {
        this._parentWrapper.removeEventListener(
          "mouseenter",
          this._boundHandlers.mouseenter
        );
      }
  ...
  get showCard() {
    return (
      this._isVisible &&
      this.decoratedHoverFields &&
      this.decoratedHoverFields.length > 0
    );
  }
  /**
  ...
  get decoratedHoverFields() {
    ...
    return this.hoverFields.map((field) => {
      ...
      if (isStatusField) {
        ...
        displayValue =
          this.taskStatus && this.taskStatus.trim().length > 0
            ? this.taskStatus.trim()
            : "—";
        richTextValue = null;
      } else {
        ...
        richTextValue =
          trimmedValue && trimmedValue.length > 0 ? trimmedValue : "";
      ...
      return {
        ...
        badgeClass: isStatusField
          ? this.getStatusBadgeClass(this.taskStatus)
          : "",
        badgeStyle:
          isStatusField && this.badgeStyle === "custom"
            ? this.getStatusBadgeStyle(this.taskStatus)
            : ""
      };
    });
  }
