...
2. **Recipient Type Management**: Portal users send to "Milestone Team", team members can send to "Client" or "Milestone Team"
3. **Infinite Scrolling**: Messages load with pagination, newest messages at bottom
4. **Server-Side Search**: Full-text search with wildcard support (`*`, `%`, literal `_`)
...
For detailed documentation, see the following sub-documents:

- **[Portal Messaging System](Portal_Messaging_System.md)**: System overview, features, security, configuration, testing, and troubleshooting
- **[portalMessaging Component](portalMessaging_Component.md)**: Core Lightning Web Component documentation
- **[salesforceMessaging Component](salesforceMessaging_Component.md)**: Wrapper component for Salesforce Lightning Experience
- **[PortalMessagingController](PortalMessagingController.md)**: Apex controller documentation with method details and implementation

### Quick Reference

**Components**:
- `portalMessaging`: Core messaging component (`force-app/portal/main/default/lwc/portalMessaging/`)
- `salesforceMessaging`: Wrapper for Salesforce context (`force-app/main/default/lwc/salesforceMessaging/`)

**Apex Controller**:
- `PortalMessagingController`: Main controller (`force-app/portal/main/default/classes/Portal/PortalMessagingController.cls`)

**Custom Object**:
- `Message__c`: Message object with fields for body, sender, recipient type, context, and status flags
