...
export default class TaskHoursMetrics extends LightningElement {
  ...
  renderChart() {
    ...
    if (
      !this.hoursData.hoursByStatus ||
      this.hoursData.hoursByStatus.length === 0
    ) {
      return;
    }
    ...
    const sortedHoursByStatus = [...this.hoursData.hoursByStatus].sort(
      (a, b) => {
  ...
  get varianceClass() {
    ...
    const varianceType =
      this.variance >= 0 ? "variance-favorable" : "variance-unfavorable";
