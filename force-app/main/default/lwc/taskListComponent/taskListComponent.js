...
export default class TaskListComponent extends NavigationMixin(
  LightningElement
) {
  @api accountId;
  ...
  buildColumns() {
    ...
    const dynamicColumns = this.fieldSetDefinitions.map((definition) =>
      this.createColumnFromDefinition(definition)
    );
  ...
  getColumnFieldName(definition) {
    ...
    const sanitizedApiName = definition.apiName
      ? definition.apiName.replace(/\./g, "_")
      : "field";
    ...
    const processedTasks = this.rawTaskData.map((task) => {
      ...
      if (this.fieldSetDefinitions.length) {
        this.fieldSetDefinitions.forEach((definition) => {
          ...
          row[definition.columnFieldName] = this.getFieldDisplayValue(
            task,
            definition
          );
        });
  ...
  getFieldDisplayValue(task, definition) {
    ...
    if (
      (definition.dataType || "").toUpperCase() === "PERCENT" &&
      value !== null &&
      value !== undefined
    ) {
      return value / 100;
