...
export default class TaskContextPanel extends NavigationMixin(
  LightningElement
) {
  /**
  ...
  wiredRecord({ error, data }) {
    ...
    if (data && this._recordWireInitialized) {
      ...
      if (timeSinceLastRefresh >= minRefreshInterval) {
        ...
        // eslint-disable-next-line @lwc/lwc/no-async-operation -- debounce LDS-driven refreshes after quick actions
  ...
  _notifyContentChange() {
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-operation -- yield to LWC reactivity before dispatching
    setTimeout(() => {
      this.dispatchEvent(
  ...
  _processWireData(wireData) {
    ...
    const dependentTasks = this._extractTaskArray(
      wireData.dependentTasks || []
    );
  ...
  _extractTaskObject(taskProxy) {
    ...
    const task = {
      ...
      relationshipId: taskProxy.relationshipId
        ? String(taskProxy.relationshipId)
        : null,
  ...
  _extractTaskArray(tasksProxy) {
    ...
    return tasksProxy
      .map((taskProxy) => {
        ...
        if (task) {
          task.statusBadgeClass = this.getStatusBadgeClassForStatus(
            task.status
          );
          task.statusBadgeStyle = this.getStatusBadgeStyle(task.status);
          task.priorityBadgeClass = this.getPriorityBadgeClassForPriority(
            task.priority
          );
  ...
  _handleError(error) {
    this._error =
      typeof error === "string"
        ? error
        : "An error occurred loading task context";
    this._dependencyData = this._createEmptyDependencyData();
  ...
  getStatusBadgeStyle(status) {
    if (
      !status ||
      !this.statusColors ||
      Object.keys(this.statusColors).length === 0
    ) {
      return "";
    }
  ...
  openLinkTaskModal() {
    ...
    if (modal) {
      ...
      this.showToast(
        "Error",
        "Unable to open Link Task modal. Please refresh the page.",
        "error"
      );
    }
  }
  ...
  async handleDeleteRelationship(relationshipId) {
    ...
    // Confirm deletion
    // eslint-disable-next-line no-alert, no-restricted-globals -- native confirm until LightningConfirm is adopted
    ...
    try {
      ...
      const errorMessage =
        error.body?.message ||
        error.message ||
        "An error occurred while deleting the relationship";
