...
export default class TaskReviewStatusMetrics extends NavigationMixin(
  LightningElement
) {
  @api accountId;
  ...
  renderCharts() {
    ...
    const clientDevCanvas = this.template.querySelector(
      "canvas.clientDevChart"
    );

    // Only retry if we have data but no canvas elements found
    if (
      (this.hasPMData && !pmCanvas) ||
      (this.hasClientData && !clientCanvas) ||
      (this.hasClientDevData && !clientDevCanvas)
    ) {
          ...
          onClick: (event, elements) => {
            if (elements.length > 0) {
              ...
              if (index === 1) {
                ...
                self.navigateToReviewListView(
                  "Pending_Review_PM_Code_Reviewer"
                );
              } else {
    ...
    if (clientDevCanvas && this.hasClientDevData) {
      ...
      const clientDevApproved =
        this.reviewData.clientDevelopmentApprovedCount || 0;
      const clientDevPending =
        this.reviewData.clientDevelopmentPendingCount || 0;
          ...
          onClick: (event, elements) => {
            if (elements.length > 0) {
              ...
              if (index === 1) {
                ...
                self.navigateToReviewListView(
                  "Backlog_Ready_for_Client_Review"
                );
              } else {
  ...
  get hasPMData() {
    return (
      this.reviewData && (this.pmApprovedCount > 0 || this.pmPendingCount > 0)
    );
  }
  ...
  navigateToReviewListView(listViewApiName) {
    ...
    const accountIdToFilter =
      this.effectiveAccountIds.length > 0 ? this.effectiveAccountIds[0] : null;
